...
sleep 10s
docker run --name rust-talk --rm -v "$current_dir":/work -p3000:3000 --link rust-postgres:postgres --link rust-memcached:memcached rust-talk bash -c '/root/.cargo/bin/diesel migration run ; cargo run'
read
